#file name picmeServer.py
import socket
import threading
from PicmeDB import PicmeDB
import os
import base64
...
class PicmeServer:
    def __init__(self, host=config.SERVER_HOST_IP, port=config.SERVER_PORT):
        ...
        self.picme_db = PicmeDB()
        self.picme_db.create_tables()
    ...
    def handle_client(self, conn, addr):
        ...
        while True:
            ...
            if command == "validate":
                ...
                response = self.picme_db.validate_user(username, password)

            elif command == "SaveSignupDetails":
                username, password, email, id, sum = args
                self.picme_db.save_user(username, password, email, id, sum)
                response = True  # or any other value to indicate success

            elif command == "ReadSignup":
                username = args[0]
                user_data = self.picme_db.get_user(username)
